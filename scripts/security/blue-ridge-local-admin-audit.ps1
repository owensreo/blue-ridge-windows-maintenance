...
try{
 ...
 $rows=foreach($m in $members){$local=$null;if($m.PrincipalSource -eq 'Local'){$n=($m.Name -split '\\')[-1];$local=Get-LocalUser -Name $n -ErrorAction SilentlyContinue};[pscustomobject]@{Computer=$env:COMPUTERNAME;Name=$m.Name;ObjectClass=$m.ObjectClass;PrincipalSource=$m.PrincipalSource;Enabled=if($local){$local.Enabled}else{$null};LastLogon=if($local){$local.LastLogon}else{$null};PasswordLastSet=if($local){$local.PasswordLastSet}else{$null};PasswordExpires=if($local){$local.PasswordExpires}else{$null};Description=if($local){$local.Description}else{$null}}}
