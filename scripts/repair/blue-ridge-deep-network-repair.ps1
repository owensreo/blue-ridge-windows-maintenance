...
try{
 ...
 Clear-DnsClientCache;nbtstat -R|Out-Null;nbtstat -RR|Out-Null;arp -d * 2>$null;Ok 'Cleared DNS, NetBIOS, and ARP caches.'
 ...
 if($RestartComputer){Restart-Computer -Force}else{Info 'A restart is recommended when Winsock, TCP/IP, or adapters were reset.'}
